...
#==============================================================================
# FUNCTION - field_line_function
#==============================================================================
#> 
#> DESCRIPTION:
#> Function to be evaluated in Runge-Kutta.
#> 
#> INPUT/OUTPUT:
#> - [IN]  grid_points - original 3D grid for interpolation
#> - [IN]  B_phi       - toroidal magnetic field for targets
...
#> - [IN] limiter_Z         - vertical position of limiter transition (optional)
#> - [IN] limiter_R         - radial position of limiter transition (optional)
#> - [IN] plot_initial      - boolean for plotting initial target distributioni
#> - [IN] plot_limiter      - boolean for plotting limiter 
#> 
#> EXAMPLE:
#> - Example:
#>
...
#> 
#==============================================================================
def plot_Poincare_Lc(filename, targets_Z, intersect_Z, intersect_R, connection_length, limiter_Z, limiter_R, plot_initial, plot_limiter):
	...
	# - compute number of intersection planes where data was saved
	nplanes = len(intersect_R[0,:])
	...
	# - setting ad hoc axis limits
	plt.axis([2.2, 5.6, -1.7, 1.7])
	
	# - plotting loop for targets intersecting with \phi = 0
	for target in range(0, ntargets):
		
		# - choosing divergin colormap
		cm = plt.cm.get_cmap('RdYlBu')
		
		# - plotting arrays in loop
		R = intersect_R[target,1:nplanes]
		Z = intersect_Z[target,1:nplanes]
		
		# - color array depending on connection length value
		color = connection_length[target]*np.ones(np.shape(R))
		...
		plt.scatter(R, Z, c=color, vmin=low, vmax=high, cmap=cm, s=size, alpha=transparency, marker='.')
	...
	if 'matrix' in filename:
		...
		# - loop over horizontal lines 
        	# 1) total lines
        	ntargets_divisor = 1
        	# 2) from top to middle
		#ntargets_divisor = 2
        	# 3) an ad hoc number
        	#ntargets_divisor = 10

        	nlines = int(ntargets / ntargets1D / ntargets_divisor)
        	...
        	for line in range(nlines):
        		...
        		# - plotting arrays in loop
        		R  = intersect_R[start_index:end_index,0]
        		Lc = connection_length[start_index:end_index]
        	
        		# - invoque color
        		color = cmap(line / (nlines - 1))
        	
        		# - radial intersection vs. connection length
        		plt.plot(R, Lc, '-', linewidth=2, alpha=transparency, color=color, label=f'Z = {targets_Z[int(start_index / ntargets1D)]}')
        		plt.xlabel('R (m)')
        		plt.title('Radial connection length')
        		
        	# - to show legend with singular values (deprecated)
        	#plt.legend(loc='upper right')
        	
        	# - to show legend with colorbar
        	sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=min(targets_Z), vmax=max(targets_Z)))
        	sm.set_array([])  # You need to set an empty array for the ScalarMappable
        	cbar = plt.colorbar(sm, label='Z (m)')
			
		# - to show connection length for each horizontal line (optional)
		#plt.show()
			
	# - for horizontal line
	elif 'horizontal' in filename:

		plt.plot(intersect_R[:,0], connection_length, '-', linewidth=1, alpha=transparency)
		plt.xlabel('R (m)')
		plt.title('Radial connection length')
	...
	# - for vertical line
		...
		plt.plot(intersect_Z[:,0], connection_length, '-', linewidth=1, alpha=transparency)
		plt.xlabel('Z (m)')
