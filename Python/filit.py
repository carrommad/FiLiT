...
intersect_R[:, 0] = initial_targets_R
intersect_Z[:, 0] = initial_targets_Z
...
for particle in range(ntargets):
	...
	while turn <= nturns:
		...
		if cross == 1:
			...
			# - save positions
			intersect_Z[particle, turn] = np.copy(dummy0[1])
			intersect_R[particle, turn] = np.copy(dummy0[2])
...
if tar_mode == 'matrix':
	...
	for line in range(ntargets % ntargets1D):
