...
from field_line_mod import read_data_hint, read_limiter, create_targets, \
						   grid_neighbours, interpolate_field, step_phi, \
						   delete_outer_particles, delete_outer_particles_limiter, \
						   save_intersection
...
ntargets1D   =  200              # Number of targets per line

nturns       =  1000             # Number of turns to the reactor

step_phi_mm  =  5                # Toroidal step in mm, for major radius R0
...
if lim_mode == 1:
	...
	dataset = nc.Dataset(limpath)
	limiter = read_limiter(dataset)
...
# - recreating original grid: careful with the order [phi,z,r] of original grids
...
# - for a given toroidal step, compute grid density only if bigger than original
n_grid_phi_enhanced = max(n_grid_phi, int(2 * np.pi * R0 * 1000 / step_phi_mm))

# - toroidal differential
delta_phi = 2 * np.pi / (n_grid_phi_enhanced - 1) / mtor
...
for step in range(nsteps):
	...
	#==============================================================================
	# a.            F U L L   T A R G E T S   P R E V I O S   S T E P
	#==============================================================================
	# - full copy of particles of previous iteration, modifying only active particles
	...
	angle    = 2 * np.pi * fraction / n_grid_phi_enhanced
	...
	#==============================================================================
	# - full copy of only active targets prior to deletion for connetion length computation
	...
	if lim_mode == 1:
		...
		# - stack together [phi,Z,R] to obtain 3D positions and transpose to coincide with dimensions of fields
		targets = np.vstack([targets_phi, targets_Z, targets_R])
		targets = np.transpose(targets)
	...
	# - metric in cyllindrical coordinates between k and k-1 with element-wise operations
...
# - declare dimensions (I could have used original targets but it is somehow modified in main loop)
