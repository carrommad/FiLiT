...
for target in range(ntargets):
	...
	"""
	plt.plot(plot_intersect_R[target,:], plot_intersect_Z[target,:], 'o', markersize=0.5)
	"""
	
	plt.scatter(plot_intersect_R[target,:], plot_intersect_Z[target,:], c=plot_connection_length[target]*np.ones(np.shape(plot_intersect_R[target,:])), cmap=cm, s=0.1)
	
	
	
plt.colorbar()
# - plotting initial targets
plt.plot(plot_intersect_R[:,0], plot_intersect_Z[:,0], 'g*', markersize=1.5)
...
if True:#lim_mode == 1:
	...
	# - plot limiter line
	plt.plot(plot_limiter_R, plot_limiter_Z, 'ko', markersize=0.5)
...
if tar_mode == 'matrix':
	...
	for line in range(int(ntargets / ntargets1D)):
		...
		plt.plot(plot_intersect_R[start_index:end_index,0], plot_connection_length[start_index:end_index], '-')
		plt.xlabel('R (m)')
...
# - for vertical line
	...
	plt.plot(plot_intersect_R[:,0], connection_length, '-')
	plt.xlabel('R (m)')
	plt.title('Radial connection length')
