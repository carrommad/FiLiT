...
#>
#> TO DO:
#> - Parallel version still not working!!!
#> -
#> 
#------------------------------------------------------------------------------
import numpy as np
...
# - condition for serial (0) and parallel (1) computation of lower level loops
parallel = 0
...
if parallel == 1:
	...
	dask.config.set(num_workers=num_cores, threads_per_worker=1, scheduler='processes')
...
#> 
#> TO DO:
#> - Parallel version doesn't work properly!!!
#> - 
#> 
#------------------------------------------------------------------------------
if parallel == 0:
	...
	def grid_neighbours_aux(m, targets_R, targets_Z, grid_R, grid_Z):
		...
		return [r, z, targets_R[m] - grid_R[r], targets_Z[m] - grid_Z[z]]
	...
	def grid_neighbours(targets_R, targets_Z, grid_R, grid_Z):
		...
		for m in range(len(targets_R)):
		    process = dask.delayed(grid_neighbours_aux)(m, targets_R, targets_Z, grid_R, grid_Z)
		    delayed_results.append(process)
		
		#with ProgressBar(): # if not commented: indent next line
		results = dask.compute(*delayed_results)
		    
		results = np.array(results)
		index_R, index_Z, dist_R, dist_Z = results[:,0], results[:,1], results[:,2], results[:,3]
...
if parallel == 0:
	...
	#------------------------------------------------------------------------------
	def interpolate_field_aux(m, index_R, index_Z, dist_R, dist_Z, F_grid, step_R, step_Z):	
	...
	def interpolate_field(index_R, index_Z, dist_R, dist_Z, F_grid, step_R, step_Z):
		...
		for m in range(len(index_R)):
			process = dask.delayed(interpolate_field_aux)(m, index_R, index_Z, dist_R, dist_Z, F_grid, step_R, step_Z)
			delayed_results.append(process)
			
		#with ProgressBar(): # if not commented: indent next line
		results = dask.compute(*delayed_results)
			
		F_inter = np.array(results)
